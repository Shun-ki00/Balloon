...
class StartTextUI : public UIObject
{
public:
	// オブジェクトのアクティブ設定
...
public:
	...
	// コンストラクタ
	StartTextUI(IObject* root, IObject* parent, IObject::ObjectID objectID,
		const DirectX::SimpleMath::Vector3& position,
		const DirectX::SimpleMath::Quaternion& rotation,
		const DirectX::SimpleMath::Vector3& scale);

	// デストラクタ
	~StartTextUI() override = default;
