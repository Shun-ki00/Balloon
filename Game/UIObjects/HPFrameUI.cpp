...
HPFrameUI::HPFrameUI(IObject* parent, IObject::ObjectID objectID,
...
/// 初期化処理
/// </summary>
void HPFrameUI::Initialize()
{
	// テクスチャサイズを取得する
	float width, height;
	Resources::GetInstance()->GetTextureResources()->GetTextureSize(TextureKeyID::HPGageFrame, width, height);
	...
	// 初期化
	m_renderableObject->Initialize(vertexBuffer, TextureKeyID::HPGageFrame, TextureKeyID::Rule ,PS_ID::UI_PS);
...
void HPFrameUI::Update(const float& elapsedTime)
{
	// Transformの更新処理
	m_transform->Update();
...
void HPFrameUI::Attach(std::unique_ptr<IObject> object)
{
...
void HPFrameUI::Detach(std::unique_ptr<IObject> object)
{
...
void HPFrameUI::OnMessegeAccepted(Message::MessageData messageData)
{
...
void HPFrameUI::OnKeyPressed(KeyType type, const DirectX::Keyboard::Keys& key)
{
