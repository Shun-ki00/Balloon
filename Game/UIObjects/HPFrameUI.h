...
class HPFrameUI : public UIObject, public IComposite
{
public:
	// オブジェクトのアクティブ設定
...
public:
	...
	// コンストラクタ
	HPFrameUI(IObject* parent, IObject::ObjectID objectID,
		const DirectX::SimpleMath::Vector3& position,
		const DirectX::SimpleMath::Quaternion& rotation,
		const DirectX::SimpleMath::Vector3& scale);

	// デストラクタ
	~HPFrameUI() override = default;
