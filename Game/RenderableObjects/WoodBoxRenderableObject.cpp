...
WoodBoxRenderableObject::WoodBoxRenderableObject(const bool& isActive, DirectX::Model* model)
	:
	m_isAcitve(isActive),
	m_model(model),
	m_baseMap{},
...
void WoodBoxRenderableObject::Initialize(const PBRLitConstantBuffer& constants)
{
	// 定数バッファに送るデータの初期値設定
...
void WoodBoxRenderableObject::Update(ID3D11DeviceContext* context, const DirectX::SimpleMath::Matrix& worldMatrix)
{
	// ワールド行列を更新する
...
ID3D11Buffer* WoodBoxRenderableObject::GetConstantBuffer() const
{
